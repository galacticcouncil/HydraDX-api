...
const sqlQueries = yesql(path.join(dirname(), "queries/hydration-web/v1/stats"), {
  type: "pg",
});

export async function cacheHydrationWebStatsJob(
  sqlClient,
  redisClient
) {
  ...
  // tvl
  const tvlQuery = await fetchFromSql(sqlClient, sqlQueries.statsTvl({ asset: null }));
  ...
  // assets_count
  const assetsCount = await fetchFromSql(sqlClient, sqlQueries.statsAssetsCount());
  result['assets_count'] = assetsCount[0]['count'];

  // accounts_count
  const accountsCount = await fetchFromSql(sqlClient, sqlQueries.statsAccountsCount());
